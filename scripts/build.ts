...
async function getGitHash(): Promise<string> {
	try {
		const cmd = new Deno.Command("git", { args: ["rev-parse", "--short", "HEAD"], stdout: "piped", stderr: "null" });
...
async function build() {
	...
	// Write a temporary build info file that the app can import
	const buildInfo = `// Auto-generated by build script — do not edit\nexport const BUILD_VERSION = "${fullVersion}";\n`;
	await Deno.writeFile("build_info.ts", encoder.encode(buildInfo));
	...
	try {
		...
		try {
			await Deno.remove("build_info.ts");
		} catch { /* ignore */ }
	}
}
